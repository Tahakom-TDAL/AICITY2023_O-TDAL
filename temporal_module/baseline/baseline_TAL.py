...
def main(vid_path, vid_ids, inference_path, out_file):
    
    topk = 7
    vid_peaks_info = get_vid_dict()
    
    right_prob_files = get_prob_files(glob(inference_path+'/A2_feat_prob_01035_right_t1/*'))
    rear_prob_files = get_prob_files(glob(inference_path+'/A2_feat_prob_01010_rear_t1/*'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_file', metavar='path', required=True,
                        help='the path where the file of temporal locatization will be saved ..')
    

    args = parser.parse_args()
    vid_path = args.vid_path
    main(args.vid_path, args.vid_ids, args.probabilities_path, args.out_file+os.sep+'baseline_temp_out.txt')
