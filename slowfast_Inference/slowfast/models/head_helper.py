...
class ResNetBasicHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for pathway in range(self.num_pathways):
            ...
            #print("inputs[pathway]",inputs[pathway].shape)
            pool_out.append(m(inputs[pathway]))
            
        #print("pool_out[0]",pool_out[0].shape)
        #print("pool_out[1]",pool_out[1].shape)  
        x = torch.cat(pool_out, 1)
        # (N, C, T, H, W) -> (N, T, H, W, C).
        x = x.permute((0, 2, 3, 4, 1))
        # Perform dropout.
        #print('before mean Features shape = ', x.shape )
        features = x.mean([1, 2, 3])
        #print('after mean Features shape = ', features.shape )
        
        #features = features.cpu().detach().numpy()
        #features = x
        #np.savez("feature_map.npz", features)
        ...
        #propaplties = x.cpu().detach().numpy()
        #np.savez("propaplties.npz", propaplties)
...
class X3DHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #print('before mean Features shape = ', features.shape )
